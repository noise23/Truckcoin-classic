 ...
 *                2 (1, +1 because bo_tth bit 2 and bit 4 are unset) non-zero bitvector bytes follow)
...
public:
    ...
    int GetBlockHeight() const {
        ...
        if(vtx.size()) {
            /* Serialised CScript */
            std::vector<uint8_t>::const_iterator scriptsig = vtx[0].vin[0].scriptSig.begin();
            uint8_t i, scount = scriptsig[0];
            /* Optimise: nTime is 4 bytes always,
             * nHeight must be less for a long time;
             * check against a threshold when the time comes */
            if(scount < 4) {
                int height = 0;
                uint8_t *pheight = (uint8_t *) &height;
                for(i = 0; i < scount; i++)
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // calculate total coin age spent in block
